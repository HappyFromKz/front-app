<template>
  <Button @click="openModal('Create')" style="margin-bottom: 20px;"
  ...
  <DataTable :value="products" tableStyle="min-width: 50rem">
    ...
    <Column field="category" header="Category">
      <template #body="{data}">
        <Button @click="openModal('Update', data.id)"
                type="button" class="p-button-success mx-2">Change</Button>
        <Button @click="deleteClient(data.id)" type="button" class="p-button-danger"
  ...
  <Dialog v-model:visible="visible" modal header="Header" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '60vw', '575px': '75vw' }">
    ...
    <Dropdown v-model="modalData.category_id" :options="[{id: null, name:'Выберите категорию'}, ...categories]" optionLabel="name" optionValue="id" placeholder="Выберите категорию" class="w-full md:w-14rem" />
    <template #footer>
